...
def main():
    ...
    fr.close()
    f = open('RunLog','a')
    print('****************splite number'+str(exc)+'*********************',file = f)
    print('MerkleTree root: '+Root,file = f)
    print('Proof Decommitment Prepare..........',file = f)
    print('Tree Node: '+str(len(tree)),file = f)
    time_total = 0;
    for excnum in range(0,exc):
        if excnum in splite.keys():
            start = time.time()
            ...
            print("Proof_"+str(excnum)+"_flag: "+str(flag),file = f)
            print ("Proof_"+str(excnum)+"_Time: "+str(end-start)+"s",file = f)

    print('End Verify..........',file = f)
    print('Total time: '+str(time_total),file = f)
